...
var tl = gsap
  ...
  const decider = answer => {
    switch (answer) {
        case 'yes':
            mainText.textContent = 'Are you sure?';
            setTimeout(() => {
                const confirmation = prompt('Are you sure? (yes/no)');
                switch (confirmation) {
                    case 'yes':
                      mainText.textContent = "Okay, you've made a choice";
                      btnRight.classList.add('disabled')
                      btnLeft.classList.add('disabled')

                      let countdown = 3;
                      const countdownInterval = setInterval(() => {
                          mainText.textContent = `Redirecting in ${countdown}...`;
                          countdown--;
                          if (countdown < 0) {
                              clearInterval(countdownInterval);
                              window.location.href = 'firework/happy__birthday.html';
                          }
                      }, 1000);
                        break;
                    case 'no':
                        mainText.textContent = 'You changed your mind?';
                        break;
                    default:
                        mainText.textContent = 'Please respond with yes or no.';
                        break;
                }
            }, 1000);
            break;
        case 'no':
          mainText.textContent = "I don't give a shit!";
          btnRight.classList.add('disabled')
          btnLeft.classList.add('disabled')
          ...
          break;
        default:
            mainText.textContent = 'Please choose yes or no.';
            break;
    }
};
...
btnRight.addEventListener('click', () => decider('yes'));
btnLeft.addEventListener('click', () => decider('no'));
